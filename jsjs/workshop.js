...
const Workshop = (() => {
  ...
  function updateRecipeList() {
    ...
    for (const [catId, catName] of Object.entries(categories)) {
      const recipes = getRecipesByCategory(catId);
      if (recipes.length === 0) continue;
      html += `<h5 class="mt-3 mb-2">${catName}</h5>`;
      html += recipes.map(recipe => {
        const canCraftNow = canCraft(recipe.id);
        const requirementsList = Object.entries(recipe.requirements)
          .map(([rid, amt]) => {
            const r = Resources.RESOURCE_TYPES[rid];
            const playerRes = Resources.getPlayerResources();
            const has = playerRes[rid] || 0;
            return `<small class="${has >= amt ? 'text-success' : 'text-danger'}">${r.name}: ${has}/${amt}</small>`;
          })
          .join(', ');

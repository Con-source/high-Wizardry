/**
 * Wizard City - UI Bridge and Debug Overlay
...
(function() {
        ...
        loc.actions.forEach(action => {
          ...
          btn.addEventListener('click', () => {
            if (window.WizardCity?.Game) {
              window.WizardCity.Game.performAction(action.id);
            }
          });
  ...
  const DebugOverlay = {
    init(gameInstance) {
      ...
      `;
      overlay.innerHTML = `
        <div>⚙️ <b>Wizard City Debug</b></div>
      ...
      setInterval(() => {
        if (!window.WizardCity?.Game) return;
        const game = window.WizardCity.Game;
  ...
  window.addEventListener('DOMContentLoaded', () => {
    if (window.WizardCity?.Game) {
      WizardUI.showNotification('Welcome to Wizard City!', { type: 'success' });
      GameUI.update(window.WizardCity.Game.state.state || window.WizardCity.Game.state);
      DebugOverlay.init(window.WizardCity.Game);
    }
